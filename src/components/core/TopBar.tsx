...
const TopBar: React.FC<TopBarProps> = ({ title = 'Dashboard', notificationCount = 1 }) => {
  return (
    <div className="h-22 flex items-center bg-white">
      <label className="text-3xl font-semibold" style={{ marginLeft: '1rem', color: '#0d4829' }}>
        {title}
      </label>

      <div className="flex-grow"></div>
       <Stack direction="row" sx={{ marginRight: '1.5rem', color: 'action.active' }}>
        <div className="relative inline-flex items-center justify-center w-10 h-10 rounded-full bg-[#0d4829]">
